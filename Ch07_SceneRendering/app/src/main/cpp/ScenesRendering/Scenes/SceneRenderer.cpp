...
void SceneRenderer::CreateResources()
{
    ...
    m_uniformBuffer_ObjectData = OpenXRCoreMgr::openxrGraphicsAPI->graphicsAPI->CreateBuffer(objectDataBufferInfo);
}
...
void SceneRenderer::SetupRenderTarget(const RenderSettings& settings)
{
    ...
    // Clear depth buffer
    OpenXRCoreMgr::openxrGraphicsAPI->graphicsAPI->ClearDepth(settings.depthImage, 1.0f);
...
void SceneRenderer::RenderObject(const struct SceneObject& object, const XrMatrix4x4f& viewProj, size_t objectIndex)
{
    ...
    OpenXRCoreMgr::openxrGraphicsAPI->graphicsAPI->SetBufferData(m_uniformBuffer_ObjectData, objectDataOffset, sizeof(ObjectRenderData), &renderData);
    ...
    OpenXRCoreMgr::openxrGraphicsAPI->graphicsAPI->SetIndexBuffer(indexBuffer);
...
void SceneRenderer::DestroyResources()
{
    for (auto& pair : m_renderPipelines)
    {
        if (pair.second)
        {
            OpenXRCoreMgr::openxrGraphicsAPI->graphicsAPI->DestroyPipeline(pair.second);
        }
    }
    ...
    {
        OpenXRCoreMgr::openxrGraphicsAPI->graphicsAPI->DestroyBuffer(m_uniformBuffer_ObjectData);
